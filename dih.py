...
class SearchManager:
    def __init__(self):
        ...
        self.executor = concurrent.futures.ThreadPoolExecutor(max_workers=MAX_CONCURRENT_SEARCHES)
    ...
    def create_search_script(self, user_id: int, search_term: str, limit: int) -> str:
        ...
        # Use grep with line buffering and limit output
        script_content = f"""#!/bin/bash
export LC_ALL=C
grep -i -a -h -m {limit} -F "{safe_term}" "{DB_PATH}" 2>/dev/null | head -n {limit} || true
"""
    ...
    async def execute_search(self, user_id: int, search_term: str, limit: int, update_msg) -> List[str]:
        async with self.search_semaphore:
            ...
            try:
                ...
                status_task = asyncio.create_task(self._update_status(update_msg))
                
                # Run grep in thread pool to avoid blocking
                loop = asyncio.get_event_loop()
                
                def run_grep():
                    try:
                        # Run grep command and collect output with timeout
                        ...
                        logger.error(f"grep timeout for user {user_id}")
                        return []
                    except Exception as e:
                        logger.error(f"grep error for user {user_id}: {e}")
                        return []
                
                # Execute in thread pool
                lines = await loop.run_in_executor(self.executor, run_grep)
    ...
    async def _update_status(self, update_msg):
        """Update status message periodically"""
        start_time = time.time()
        try:
            while True:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("search email/username", callback_data="search_email")],
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "profile":
        ...
        help_text = (
            ...
            "email: search by email address or username\n"
...
async def handle_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        result_limit = 50000 if is_premium else 10000
        ...
        # Prepare files in thread pool
        loop = asyncio.get_event_loop()
        files_to_send = await loop.run_in_executor(
            search_manager.executor, 
            prepare_files, 
            content, 
            filename
        )
        ...
        context.user_data.pop('search_mode', None)
        # Clean up any remaining active search reference
        if str(user.id) in db.active_searches:
            del db.active_searches[str(user.id)]
