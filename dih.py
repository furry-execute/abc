...
TEXTS = {
    ...
    "ku": {
        ...
        "searching": "🔍 گەڕان لە هەموو داتabەیسەکاندا...",
...
def format_aman_result(aman_data: Dict, lang: str = "ku") -> str:
    ...
    if lang == "ku":
        ...
        result_text = (
            ...
            f"• **ژمارا ترومbێلێ:** {aman_data.get('car_number', 'N/A')}\n"
...
async def handle_search_query(message: Message, state: FSMContext):
    ...
    search_msg = await message.answer(get_text(user_id, "searching"))
...
async def validate_tokens_command(message: Message):
    ...
    if user_id != ADMIN_USER_ID:
        await message.answer("❌ تەنیا ئەدمین دەتوانێت ئەم فرمانە بەکاربهێنێت.")
        return
    
    await message.answer("🔄 دەستپێکردنی پشکنینی تۆکنەکانی Truecaller...")
...
async def reload_tokens_command(message: Message):
    ...
    await message.answer(f"✅ تۆکنەکان بارکرانەوە!\nکۆی گشتی: {len(truecaller_tokens)}")
