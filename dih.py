...
class BotDatabase:
    def __init__(self):
        ...
        self.active_searches: Dict[str, bool] = {} # Tracks who is currently searching
        self.load_all()
...
def heavy_processing_task(header, lines, filename):
    """
    Helper function to run CPU-heavy tasks (joining strings, zipping)
    in a separate thread to avoid blocking the bot.
    """
    content = header + "\n".join(lines)
    return prepare_files(content, filename)
...
async def handle_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Lock user to prevent concurrent searches
        db.active_searches[str(user.id)] = True
        ...
        # Release lock immediately after search to prevent sticking
        if str(user.id) in db.active_searches:
        ...
        await status_msg.edit_text(f"found {len(lines)} lines! preparing file... (this might take a sec)")
        
        safe_term = re.sub(r'[^\w]', '_', search_term)[:30]
        filename = f"{BOT_USERNAME}_{context.user_data['search_mode']}_{safe_term}.txt"
        
        # --- FIXED BLOCK ---
        # Run heavy processing in background thread so bot doesn't freeze
        ...
        # Final cleanup ensuring lock is released
        if str(user.id) in db.active_searches:
            del db.active_searches[str(user.id)]
        context.user_data.pop('search_mode', None)
